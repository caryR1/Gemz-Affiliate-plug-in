...
Plugin `GAS_VERSION` **2.22.2** / `GAS_DB_VERSION` 19, live on Solar and matching repo `main` (corrected 2026-09-19; earlier sections of this file were written at 2.21.x and carry their own dates). Automated tests: **PHPUnit 66 tests / 168 assertions**, green on the server. Sensitive data: encrypted at rest and fail-closed (F11, F12).
...
| Automated tests | IT | PHPUnit **66 tests / 168 assertions** green on the server 2026-09-19 (was 31/78 when this row was written; later additions: credit window and IP rule, encryption, fail-closed writes) (pure-logic suite; run from a scratch dir over SSH with `composer install`, then deleted). Covers payout math, rounding at the $425 pool, a 500-pool float-noise guard, coverage matching, estimate ranges. Does not cover WordPress-dependent paths (hooks, REST, DB) — those rely on live/staging checks. |
...
- **Key rotated 2026-09-19** (same day as creation, at Cary's request, after part of the first key appeared in a screenshot): done while zero encrypted values existed, so nothing had to be re-encrypted. Server and local key fingerprints were compared and match, the old key is absent from `wp-config.php`, a live encrypt/decrypt round trip passed, and the backup that contained the old key was deleted. **There is still no rotation tool:** once real data is stored, changing the key would orphan it. A future tool would decrypt with the old key and re-encrypt with a new one (the `gasenc1` prefix leaves room for a key id).
- **Behavior without a valid key:** **SUPERSEDED by F12 (v2.22.2): saves are now REFUSED, never stored as plaintext.** (The original 2.22.0/2.22.1 behavior was a plaintext fallback with a red Ledger warning.) Reads of an encrypted value with a missing or wrong key return empty (never ciphertext or a crash), and the tax export marks the tax ID `UNREADABLE (encryption key missing or changed)`. Legacy plaintext still reads normally, and `GAS_Crypto::migrate_plaintext()` (idempotent; verifies each value decrypts back before writing) converts old rows. The Ledger page shows how many plaintext values remain.
...
- **Extended 2026-09-19 (v2.22.1, Cary: "yes") to the PayPal client secret and the Wise API token in `wp_options`.** Stored encrypted through `GAS_Crypto::update_secret_option()` and read back through `get_secret_option()` in the PayPal and Wise payout classes and the automated run. The admin API-settings form no longer writes a saved secret back into the page (previously the saved value was in the HTML of a password field, visible in page source to any admin): the field is blank with a "Saved, leave blank to keep" placeholder, a blank submit keeps the stored value, and a "Remove" checkbox clears it. Client IDs and profile IDs are identifiers and stay plain. Legacy plaintext values still read and are converted by `migrate_plaintext()`. Solar had no PayPal or Wise credentials saved, so nothing needed converting; **when live PayPal credentials are entered, enter them in this form so they are encrypted from the start.** Live test through the real form handler: encrypted storage with no plaintext row, never present in the page HTML, blank keeps, Remove clears, legacy plaintext migrates; original option state restored exactly. PHPUnit 51 tests / 116 assertions. Still not encrypted: the payout-cron URL token (`gas_automated_payout_token`, shown on the Ledger page by design).
- **Not covered / follow-ups:** signup IP and phone numbers are not encrypted; any other site running this plugin (for example Home) needs its own `GAS_DATA_KEY` and a run of `GAS_Crypto::migrate_plaintext()` after it deploys v2.22.0; no key-rotation tool (the `gasenc1` prefix leaves room to add one).

**F12. Sensitive data now fails closed (v2.22.2, live 2026-09-19; requested by ChatGPT's security follow-up, approved by Cary in chat: "Yes, refuse the save").**
- **Rule:** if `GAS_DATA_KEY` is missing, invalid, or encryption is otherwise not working, the plugin **never stores new sensitive data as plaintext**. It refuses the save, shows a clear message, and changes nothing. Covers tax ID, tax legal name and country; PayPal email, Wise name/account/routing, payment notes; buyer cash-back payment details; and the PayPal client secret and Wise API token.
- **How:** `GAS_Crypto::encrypt()` now returns `false` (never the plaintext) when a non-empty value cannot be encrypted; empty values still return `''` so clearing a field works. `GAS_Crypto::operational()` is true only when a valid key exists AND a real encrypt/decrypt round trip works. `GAS_Payouts::save_tax_info()` and `save_details()` encrypt every sensitive value first and write **all or nothing** (a refusal leaves earlier encrypted data and even the non-sensitive fields untouched, and they return `false` with `GAS_Crypto::$refused` set). The cash-back save and `update_secret_option()` refuse the same way.
- **What people see:** an affiliate saving payment or tax info gets "Your information could not be saved because secure storage is not available on this site right now. Nothing was stored. Please contact the site administrator and try again later." A cash-back claimant sees the same as a 503 page. An admin saving an API secret is redirected with an error ("Not saved: encryption is not active ..."); the other fields on that form are saved. The Payout Ledger page keeps its status line and its red warning now says new tax IDs, payment details, cash-back details and API secrets are refused until a valid key is added.
- **Still supported:** reading legacy plaintext (with or without a key) and `GAS_Crypto::migrate_plaintext()` (which does nothing, returning `skipped = -1`, when encryption is not operational).
- **Tests:** PHPUnit now 66 tests / 168 assertions. New: missing key, invalid key, cipher failure and a working key for the primitive; refusal of tax, payment and API-secret saves with nothing written (including no partial update and no overwrite of earlier encrypted data); a validation error is not reported as a refusal; clearing fields is allowed without a key; legacy plaintext still reads. **Live tests on Solar with temporary data only, all passed and removed:** normal encrypted save and read-back; missing and invalid key refuse both saves and leave the database rows identical (no leaked plaintext); cipher failure refuses; the real affiliate tax and payment form handlers show the error and store nothing, then succeed with the real key; the admin API-secret form is refused with `secret_refused` and stores nothing (no secret text anywhere in `wp_options`), then saves encrypted with the real key; cash-back refused then encrypted; legacy plaintext reads with a broken key and migrates with the real key; settings and users restored exactly. No key or sensitive value was printed.
